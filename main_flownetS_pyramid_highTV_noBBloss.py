...
def train():
    ...
    with tf.variable_scope('loss'):
        ...
        var6 = tf.reduce_sum(tf.image.total_variation(outputs['predict_flow6']))/1e6
        var5 = tf.reduce_sum(tf.image.total_variation(outputs['predict_flow5']))/1e6
        var4 = tf.reduce_sum(tf.image.total_variation(outputs['predict_flow4']))/1e6
        var3 = tf.reduce_sum(tf.image.total_variation(outputs['predict_flow3']))/2e6
        var2 = tf.reduce_sum(tf.image.total_variation(outputs['predict_flow2']))/2e6
    ...
    with tf.variable_scope('loss_test'):
        ...
        var6 = tf.reduce_sum(tf.image.total_variation(outputs_test['predict_flow6']))/1e6
        var5 = tf.reduce_sum(tf.image.total_variation(outputs_test['predict_flow5']))/1e6
        var4 = tf.reduce_sum(tf.image.total_variation(outputs_test['predict_flow4']))/1e6
        var3 = tf.reduce_sum(tf.image.total_variation(outputs_test['predict_flow3']))/2e6
        var2 = tf.reduce_sum(tf.image.total_variation(outputs_test['predict_flow2']))/2e6
    ...
    tl.files.load_and_assign_npz_dict(name = '/Jarvis/logs/LJH/deep_video_stabilization/VS_flownetS_pyramid/fixed_ckpt/VS_flownetS_pyramid_31k.npz', sess = sess)
...
def evaluate_originalSize():
    ...
    for k in np.arange(len(test_video_list)):
        ...
        with tf.variable_scope('input'):
            ...
            ofnum = tf.placeholder('float32', [], name = 'input_OF_num')
            ofsum = tf.placeholder('float32', [batch_size, out_h, out_w, 2], name = 'input_OF_sum')
        # define model
        with tf.variable_scope('main_net') as scope:
            outputs = flownetS_pyramid(inputs,batch_size,is_train=False )
            ...
            outflow = tf.concat([tf.ones_like(outflow[:,:,:,0:1])*tf.reduce_mean(outflow[:,:,:,0],axis=[1,2]),tf.ones_like(outflow[:,:,:,1:2])*tf.reduce_mean(outflow[:,:,:,1],axis=[1,2])],3)
            outflowMean = (ofsum+outflow)/ofnum
            outputs_warpedimg = tf_warp(resizedInput, outflowMean , out_h,out_w)
            #print(outputs_warpedimg)
            #raise
        ...
        totaloutputFrame = np.zeros([int(total_frames),out_h,out_w,3])
        totaloutputOF = np.zeros([int(total_frames),out_h,out_w,2])
        #cap.set(1,0)
        #ret_unstab, frame_unstab = cap.read()
        ...
        for i in range(int(total_frames)):
            ...
            curflownum = min(i+1,3)
            curflowsum = np.zeros([out_h,out_w,2])
            for j in range(curflownum-1):
                curflowsum += totaloutputOF[i-j-1]
            curflowsum = np.expand_dims(curflowsum,axis=0)

            feed_dict = {inputs: curinput, resizedInput: np.expand_dims(cv2.cvtColor(frame_unstab, cv2.COLOR_RGB2BGR)/255.0  ,axis=0), ofnum:curflownum, ofsum:curflowsum}
            curwarpedimg = sess.run(outputs_warpedimg , feed_dict)
            totaloutputOF[i] = sess.run(outflow , feed_dict)
