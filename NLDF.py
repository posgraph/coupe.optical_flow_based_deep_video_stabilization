...
class Model:
    ...
    def build_model(self, input_holder, batch_size, reuse, scope):
        with tf.variable_scope(scope, reuse=reuse) as vs:

            #build the VGG-16 model
            vgg = self.vgg
            input_holder = input_holder * 255. - vgg16.VGG_MEAN
            ...
            self.Score = tf.reshape(self.Score, [-1,176, 176, 2])

            self.Prob = tf.nn.softmax(self.Score)
            self.Prob = tf.expand_dims(self.Prob[:, :, :, 0], axis = 3)
